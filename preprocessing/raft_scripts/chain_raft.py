...
def run(args):
    ...
    img_files = sorted(glob.glob(os.path.join(scene_dir, 'color', '*')))[args.frame_sample[0]:args.frame_sample[1]]
    num_imgs = len(img_files)
    print(f'[CHAIN RAFT] num_imgs: {num_imgs}, num_K: {args.K}')
    # pbar = tqdm(total=num_imgs*(num_imgs-1))
    pbar = tqdm(total=num_imgs*args.K)
    ...
    for i in range(num_imgs - 1):
        ...
        for j in range(i + 1, min(num_imgs, i + args.K + 1)):
            # # vis
    ...
    for i in range(num_imgs - 1, 0, -1):
        ...
        for j in range(i - 1, max(-1, i - args.K - 1), -1):
            # # vis
            imgname_j = os.path.basename(img_files[j])
            direct_flow_file = os.path.join(scene_dir, 'raft_exhaustive', '{}_{}.npy'.format(imgname_i, imgname_j))
            direct_flow = np.load(direct_flow_file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--K', type=int, default=5, help="interval of images to process")
